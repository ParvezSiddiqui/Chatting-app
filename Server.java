...
import java.net.*;           // This package is used to import "ServerSocket" class=Server and "Socket" class=Client   
import java.io.*;        // For Data input/output stream

public class Server implements ActionListener {  // Action Listener interface comes from package java.awt.event.*;
  ...
  JPanel a1;
   ...
   static JFrame f=new JFrame();    // Object of JFrame is made as we cannot extend JFrame due to validate fuction is non static
   static DataOutputStream dout;

  Server(){    // Constructor under which code will be written
  ...
  // Function to add Name
    JLabel name=new JLabel("Gaitonde"); // here name is the object used in JLabel
  ...
  // Main frame parameters
    ...
    f.setLocation(200,100);       // Frame location
  ...
  public static JPanel formatLabel (String out){
    ...
    public static void main(String args[]){
       new Server(); // anonymus object without name
      

       //Below we will do Socket programing to comunicate btwn two clases
                  //  try and catch used to avoid exceptions  
      try {
           ServerSocket skt =new ServerSocket(6001);   //6001 is Port no.
           
           while(true){           // We want msg to be send and receive infinte time so use while loop till it is true
               Socket s=skt.accept();
